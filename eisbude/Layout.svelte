...
<main class:dark={darkMode} class="font-['Roboto_Mono'] font-medium h-full dark:bg-[#1F2937] dark:text-white overflow-auto">
	<div class="p-8 flex flex-col gap-y-4 xl:px-[20vw]">
		<div class="flex justify-center mb-4">
			<i class="fa-solid fa-ice-cream text-7xl -rotate-45 text-[#E879F9]"></i>
			<div on:click={toggleDarkMode} class:toggledDark={darkMode} class="flex items-center w-16 h-8 bg-[#374151] cursor-pointer text-[#FBBF24] rounded-2xl px-2 text-lg">
				{#if darkMode}
					<i in:fly={{duration:400}} class="fa-solid fa-moon"></i>
				{:else}
					<i in:fly={{duration:400}} class="fa-solid fa-sun"></i>
				{/if}
			</div>
		</div>
